...
As a check, anything in `recount_brain` has to be in `recount2` by construction. We'll later take a deeper look at the `r length(studies_to_check)` studies present in `MetaSRA` and `recount2` yet absent from `recount_brain`.
...
From these checks, we can see that 26.6 and 44.9 percent of the `recount_brain` studies and samples are missing from `MetaSRA`, respectively.
...
discrepant_studies_samples <- map(discrepant$project, function(x) {
    ...
    data.frame(
        ...
        reported = PredictedPhenotypes_latest$reported_tissue[m2],
        project = x,
        sample_id = y,
...
discrepant <- cbind(discrepant, map_dfr(discrepant_studies_samples, function(x) {
    ...
    data.frame(
        ...
        brain_n_reported = sum(x$reported == 'Brain', na.rm = TRUE),
        ...
        brain_percent_latest = sum(x$prediction_latest == 'Brain',
            na.rm = TRUE) / nrow(x) * 100,
        brain_percent_reported = sum(x$reported == 'Brain',
            na.rm = TRUE) / nrow(x) * 100,
...
## Does it match the original criterial of at least 4 brain samples and greater than
## 70% brain samples in the study?
discrepant$criteria_original <- discrepant$brain_n_original >= 4 &
    discrepant$brain_percent_original > 70
discrepant$criteria_latest <- discrepant$brain_n_latest >= 4 &
    discrepant$brain_percent_latest > 70
discrepant$criteria_reported <- discrepant$brain_n_reported >= 4 &
    discrepant$brain_percent_reported > 70
discrepant$criteria_metasra <- discrepant$brain_n_metasra >= 4 &
...
addmargins(with(discrepant,
    table(criteria_latest, criteria_reported, criteria_metasra)))
```
...
ggplot(discrepant,
    ...
    geom_point() +
    facet_grid( ~ criteria_reported) +
    geom_abline(linetype = 3, color = 'red') +
    labs(caption = 'Panels by criteria_reported')

## just those with some TRUE criteria
ggplot(subset(discrepant,
    criteria_reported | criteria_metasra | criteria_latest),
    aes(x = brain_percent_original, y = brain_percent_latest,
        color = criteria_latest, size = number_samples,
        shape = criteria_metasra)) +
    geom_point() +
...
There are `r with(discrepant, sum(criteria_reported & !(criteria_metasra | criteria_latest)))` studies that only pass the selection criteria based on the `reported_tissue` information present in the predictions table.

```{r 'reported only'}
subset(discrepant, criteria_reported & !(criteria_metasra | criteria_latest))
```

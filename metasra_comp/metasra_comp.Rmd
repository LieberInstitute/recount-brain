...
There are `r with(discrepant, sum(criteria_reported & !(criteria_metasra | criteria_latest)))` studies that only pass the selection criteria based on the `reported_tissue` information present in the predictions table. The `reported_tissue` was extracted from SHARQ beta as described in the `phenopredict` manuscript `r citep(bib[['phenopredict']])`. 
